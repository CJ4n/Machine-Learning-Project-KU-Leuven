...
def training(env, ts, max_iterations, runs_dir, config_params):
    ...
    algo = config.build()
    best_score = 0
    history = {"archer_0": [], "archer_1": [], "sum": []}

    path_history = str(runs_dir / f"history_multi_ppo_{ts}.npy")
    ...
    for i in tqdm(range(max_iterations), desc="Training Iterations"):
        ...
        history["sum"].append(score)

        np.save(path_history, np.array(history))

        if best_score < score:
            best_score = score
            path_best = (weights_dir / f"best_{ts}_multi_ppo_{score}").resolve()
            save_result = algo.save(path_best)
            path_to_checkpoint = save_result.checkpoint.path
            print(f"Save best checkpoint '{path_to_checkpoint}'.")

        if i % 10 == 0:
            checkpoint_path = str(weights_dir / f"results_{ts}_{i}_multi_ppo_{score}")
...
class CustomWrapperNoModifiedRewards(BaseWrapper):
    def __init__(self, env, vis_obs=False, max_zombies=4, training=False):
        super().__init__(env)
...
class CustomWrapperWithModifiedRewards(BaseWrapper):
    def __init__(self, env, vis_obs=False, max_zombies=4, training=False):
        super().__init__(env)
        self.vis_obs = vis_obs
        self.max_zombies = max_zombies
    ...
    def _count_zombies(self, agent):
        obs = self.env.observe(agent)
        zombie_rows = obs[-self.max_zombies :]
        return sum(1 for zrow in zombie_rows if zrow[0] > 0)
    ...
    def step(self, action) -> None:
        ...
        pre_zombies = self._count_zombies(current_agent)

        self.env.step(action)

        post_zombies = self._count_zombies(current_agent)
        zombies_killed = pre_zombies - post_zombies

        if zombies_killed > 0:
            for agent in self.env.agents:
                self.env.rewards[agent] += 1 * zombies_killed
...
def run_grid_search():
    param_grid = {
        ...
        "rollout_length": [512],
        "max_iterations": [1000],
        "use_modified_rewards": [True, False],
    }
    ...
    base_runs_dir = package_directory / "runs" / f"grid_search_multi_{time_}"
    ...
    for i, config in enumerate(param_combinations):
        ...
        if config["use_modified_rewards"]:
            env = CustomWrapperWithModifiedRewards(env, training=True)
        else:
            env = CustomWrapperNoModifiedRewards(env, training=True)

        config_name = f"c{config['max_cycles']}_{'y' if config['use_kl_loss'] else 'n'}KL_r{config['rollout_length']}_{'mod' if config['use_modified_rewards'] else 'orig'}_rewards"
        ...
        plt.plot(history["sum"], label="cumulative reward")
        plt.legend()
        plt.xlabel("Epoches")
        plt.ylabel("Average rewards")
        plt.title(
            f"Performance of Archers (PPO) - {'Modified' if config['use_modified_rewards'] else 'Original'} Rewards"
        )
